...
async function startServer() {
  ...
  // API Route for sending admin notifications
  app.post("/api/notify-admin", async (req, res) => {
    console.log("Received admin notification request:", req.body);
    ...
    try {
      ...
      res.status(200).json({ message: "Admin notification sent", data });
    } catch (err) {
