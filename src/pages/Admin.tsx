...
export default function Admin() {
          ...
          async (snapshot) => {
            ...
            for (const docSnap of snapshot.docs) {
              ...
              if (isExpired && !data.isSuspended) {
                try {
                  updateDoc(docSnap.ref, { isSuspended: true }).catch(
                    console.error,
                  );
                } catch (e) {}
              ...
              try {
                const ordersSnap = await getDocs(
                  collection(db, `customers/${docSnap.id}/orders`),
                );
                ...
                console.error(
                  "Could not fetch orders for customer",
                  docSnap.id,
                  err,
                );
              }
  ...
  useEffect(() => {
    ...
    customers.forEach((c) => {
      const qMsgs = query(
        collection(db, "customers", c.id, "messages"),
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCustomer.name || !newCustomer.orderNumber || !newCustomer.email)
      return;
      ...
      if (customer && order) {
        fetch("/api/send-status-email", {
          ...
          .catch((err) => {
            ...
            alert(
              "Ordren blev opdateret, men e-mailen kunne ikke sendes. Tjek venligst dine API-indstillinger.",
            );
          });
      }
  ...
  const pendingOrders = customers
    ...
    .filter(
      (o) =>
        o.status === "Afventer" ||
        o.status === "I gang" ||
        o.status === "Afventer betaling",
    )
    .sort((a, b) => {
      // Prioritize "Afventer betaling" as it requires admin action for payment verification
      if (a.status === "Afventer betaling" && b.status !== "Afventer betaling")
        return -1;
      if (a.status !== "Afventer betaling" && b.status === "Afventer betaling")
        return 1;
    ...
    <div className="pt-8 pb-24 max-w-6xl mx-auto w-full px-4">
      ...
      <div className="mb-8">
        <h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
          ...
          <span className="bg-purple-500/20 text-purple-400 px-2 py-0.5 rounded-full text-sm ml-2">
          ...
          {filteredCustomers.length === 0 ? (
            <div className="text-center py-12 text-zinc-500 bg-[#111111] rounded-[2rem] border border-zinc-800 border-dashed">
              {customers.length === 0
                ...
                : "Ingen kunder matchede din søgning."}
            </div>
          ) : (
            filteredCustomers.map((customer) => (
              <div
                key={customer.id}
                className="bg-[#111111] border border-zinc-800/80 rounded-[2rem] p-6 shadow-lg overflow-hidden"
              >
                <div className="flex flex-col sm:flex-row items-center sm:items-start justify-between gap-4">
                    ...
                    {editingCustomerId === customer.id ? (
                      ...
                      <div>
                        <h3 className="text-xl font-bold text-white leading-tight flex items-center gap-2 flex-wrap">
                          {customer.name}
                          <span
                            className={`text-[10px] uppercase px-2 py-0.5 rounded-full font-bold tracking-wide ${customer.type === "regular" ? "bg-amber-500/20 text-amber-500 border border-amber-500/20" : "bg-purple-500/20 text-purple-500 border border-purple-500/20"}`}
                          >
                        ...
                        <div className="flex flex-col gap-1 items-start mt-1">
                          ...
                          <span className="text-zinc-400 font-mono text-sm bg-zinc-900 px-2 py-0.5 rounded inline-block border border-zinc-800">
                            Kode: {customer.orderNumber}
                          </span>
                          <span className="text-xs text-zinc-500">
                            Oprettet: {customer.createdAt}
                          </span>
                          {customer.expirationDate && (
                            <span className="text-xs text-amber-500 font-medium">
                              Udløber: {customer.expirationDate}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div className="flex items-center justify-between w-full sm:w-auto gap-3">
                    ...
                    <button
                      onClick={() =>
                        handleToggleSuspend(customer.id, customer.isSuspended)
                      }
                          ...
                          >
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-4">
                              ...
                              <div className="flex items-center gap-2 shrink-0 flex-wrap">
                                ...
                                <button
                                  onClick={() =>
                                    handleDeleteOrder(customer.id, order.id)
                                  }
                                  ...
                                  <div>
                                    ...
                                    {order.imageUrl && (
                                      <img
                                        src={order.imageUrl}
                                        alt=""
                                        className="mt-3 w-16 h-16 rounded-xl border border-zinc-700 object-cover"
                                      />
                                    )}
                                  </div>
                                </div>
